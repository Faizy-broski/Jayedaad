...
export function useAdminListingsViewModel(filters) {
    ...
    const setStatus = useMutation({
        mutationFn: ({ listingId, status }) => listingsRepository.setStatus(listingId, status),
        onSuccess: invalidate,
    });
    const remove = useMutation({
        mutationFn: (listingId) => listingsRepository.deleteListing(listingId),
        onSuccess: invalidate,
    });
    return {
...
export function useAdminListingDetailViewModel(listingId) {
    const queryClient = useQueryClient();
    ...
    const setStatus = useMutation({
        mutationFn: ({ status }) => listingsRepository.setStatus(listingId, status),
        onSuccess: () => {
