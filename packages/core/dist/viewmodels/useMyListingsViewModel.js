...
export function useMyListingsViewModel(filters) {
    ...
    const update = useMutation({
        mutationFn: ({ listingId, input }) => listingsRepository.updateListing(listingId, input),
    ...
    const submitForVerification = useMutation({
        mutationFn: (listingId) => listingsRepository.submitDraft(listingId),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['listings', 'mine'] }),
    });
    return {
