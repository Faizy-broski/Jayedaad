...
export function AgencyStaffCard({ staff }: { staff: AgencyStaffPreview }) {
  ...
  return (
    <div className="flex w-[220px] shrink-0 flex-col items-center gap-3 rounded-2xl border border-border bg-card p-5 text-center shadow-sm">
      ...
      <div className="min-w-0">
        ...
        {staff.title && <p className="truncate text-xs text-muted-foreground">{staff.title}</p>}
      </div>
      <div className="flex w-full gap-2">
        <a
          href="mailto:hello@jayedaad.com"
          className="flex flex-1 items-center justify-center gap-1.5 rounded-full border border-border px-3 py-2 text-xs font-medium text-muted-foreground transition-colors hover:border-primary/40"
        >
