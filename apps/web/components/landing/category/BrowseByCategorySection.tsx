...
export function BrowseByCategorySection() {
  const { propertyTypes, isLoading: taxonomyLoading } = useTaxonomyViewModel();
  ...
  const showLive = !taxonomyLoading && categories.length > 0;

  return <BrowseByCategory categories={showLive ? categories : CATEGORIES} />;
}
