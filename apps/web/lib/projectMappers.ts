...
// Static PROJECTS is only a loading/empty-state fallback now — real card
// data comes from GET /projects via projectToCardData below. Shared by the
// homepage's New Projects carousel and the /developments browse grid.
export const FALLBACK_PROJECT_CARDS: ProjectCardData[] = PROJECTS.map((project) => ({
  id: project.id,
  slug: project.slug,
  name: project.name,
  ...
  unitTypeCount: project.unitTypes.length,
  priceRangeMin: project.priceRange.min,
}));
