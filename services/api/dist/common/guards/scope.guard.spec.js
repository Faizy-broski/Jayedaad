...
describe('ScopeGuard', () => {
    function makeGuard(rolesForRoute) {
        const reflector = { getAllAndOverride: jest.fn().mockReturnValue(rolesForRoute) };
        return new scope_guard_1.ScopeGuard(reflector);
    }
    ...
    it('rejects an agent from staff-only verification routes', () => {
        const guard = makeGuard(['verification_staff', 'super_admin']);
        const ctx = buildContext({ id: '1', role: 'agent', agentId: 'agent-1' });
        expect(() => guard.canActivate(ctx)).toThrow(common_1.ForbiddenException);
    });
